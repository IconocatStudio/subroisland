	...
	if cell_target_type == CELL_TYPES.EMPTY:
		...
		if cell_obj:
			if cell_obj.obj_type == CELL_TYPES.OBJECT:
				...
				return update_overworld_obj_position(requesting_object, cell_start, cell_target)
		else:
			return update_overworld_obj_position(requesting_object, cell_start, cell_target)
...
# removes an object from children array. The object will queue_free itself, but
# we want the overworld to immediately know this cell is no longer occupied
