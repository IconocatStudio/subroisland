...
# Disable the ability for this to be interacted with, then begin the animation process
# re-enable once target destination is reached
func move_to(target_position):
	...
	# Keep the pivot where it is, because we are about to move the whole transform
	# and it will cause a glitchy animation where the sprite warps for a single frame
	# to the target location (with the transform) and then smoothly animates after
	$Pivot.position = current_pos
	# Move the pivot point from the current position to 0,0 (relative to parent transform)
	# so basically just catch up with the parent
	$Tween.interpolate_property($Pivot, "position", current_pos, Vector2(), $AnimationPlayer.current_animation_length, Tween.TRANS_LINEAR, Tween.EASE_IN)
