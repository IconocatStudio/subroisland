...
var choice_scene = load("res://Scenes/Dialog/Choice.tscn") # Base scene for que choices
## Required nodes ##
...
onready var progress = GameData # The AutoLoad script where the interaction log, quest variables, inventory and other useful data should be acessible.
...
var choice_width : int = 300 # Choice label's width
...
func update_dialogue(step): # step == whole dialogue block
	...
	# Let InputSystem know a dialog is in progress
	InputSystem.activate_dialog()
...
func next():
	...
	if next_step == '': # Doesn't have a 'next' block.
		if current.has('animation_out'):
			...
			sprite_right.modulate = white_transparent
		dialogue = null
		# Let InputSystem know to no longer handle dialog input
		InputSystem.deactivate_dialog()
		name_left.hide()
		name_right.hide()
		frame.hide()
		avatar_left = ''
